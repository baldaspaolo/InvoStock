...
const OrganizationsItem = () => {
    ...
    const fetchTabData = async () => {
      ...
      try {
        const res = await fetch(
          `${API_URL}/api/admin/organizations/${id}/${activeTab}`
        );
          ...
          <div>
            <div>
              <div>
                <h3>Osnovne informacije</h3>
                <p>
                  <strong>Ime:</strong> {selectedItem.name || "-"}
                </p>
                <p>
                  <strong>Email:</strong> {selectedItem.email || "-"}
                </p>
                <p>
                  <strong>Uloga u organizaciji:</strong>{" "}
                  {selectedItem.org_role === "admin" ? "Admin" : "Član"}
                </p>
              </div>
              <div>
                <h3>Dodatne informacije</h3>
                <p>
                  <strong>Datum pridruživanja:</strong>{" "}
                  {formatDate(selectedItem.created_at)}
                </p>
                <p>
                  <strong>Globalna uloga:</strong>{" "}
                  {selectedItem.role === "systemadmin"
                    ? "Sustavni administrator"
                    : selectedItem.role === "organization"
  ...
  const getColumns = () => {
    switch (activeTab) {
      ...
      case "members":
        return [
          { field: "name", header: "Ime", sortable: true },
          { field: "email", header: "Email", sortable: true },
          {
            field: "org_role",
            header: "Uloga",
            sortable: true,
            body: (row) => (row.org_role === "admin" ? "Admin" : "Član"),
          },
          {
            field: "created_at",
  ...
  const rowExpansionTemplate = (rowData) => {
    switch (activeTab) {
      ...
      default:
        return <div>Nema dodatnih podataka</div>;
    }
  };
  ...
  if (!organization) {
    return <p>Organizacija nije pronađena</p>;
  }
      ...
      <Panel header="Podaci o organizaciji" style={{ margin: "4% 3% 2% 3%" }}>
        <div>
          <div>
            ...
            <p>
              <strong>Adresa:</strong> {organization.address || "-"}
            </p>
          </div>
          <div>
            <p>
